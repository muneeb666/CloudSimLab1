	...
	public class edgeIoTmodel {
		...
		public static void main(String[] args) 
		{
			...
			//	Log.disable();
				...
				if(CLOUD)
				{
					moduleMapping.addModuleToDevice("Global Data Processing", "cloud"); 
					moduleMapping.addModuleToDevice("Decison Making", "cloud"); 
					moduleMapping.addModuleToDevice("Edge Handlers", "cloud");
					


					for(FogDevice device : fogDevices){
						if(device.getName().startsWith("Edging")){
							
					moduleMapping.addModuleToDevice("Edgedevices", device.getName());  
						}
					}
	  ...
	  // Cloud in Public Network
		...
		private static FogDevice addsp(String id, int userId, String appId, int parentId){
			...
			user.setLatency(10.0);  
			display.setGatewayDeviceId(sp.getId());
			display.setLatency(10.0);  
			return sp;
		}
		...
		private static Application createApplication(String appId, int userId){
			...
			application.addAppEdge("TEMP","customer", 1000, 100, "SENSED", Tuple.UP, AppEdge.SENSOR);
