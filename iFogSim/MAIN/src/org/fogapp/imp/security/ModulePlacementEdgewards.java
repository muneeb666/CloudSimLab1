...
public class ModulePlacementEdgewards extends ModulePlacement{
				...
				if(upsteamDeviceId > 0){
					if(upsteamDeviceId==deviceId){
						...
						if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()){ 
							Logger.debug("Request Transfer", " To "+moduleName+" Idle Devics " + device.getName());
						...
						} else
						{
							...
							Logger.debug("Request Placement", "of  "+moduleName+" can be transffered to "+device.getName());
						}
					}
				}else{
	...
	protected List<List<Integer>> getLeafToRootPaths(){ //getting parents in each application i.e Cloud
		...
		for(FogDevice device : getFogDevices()){
			if(device.getName().equals("WSN-Cloud"))
				cloud = device;
		}
