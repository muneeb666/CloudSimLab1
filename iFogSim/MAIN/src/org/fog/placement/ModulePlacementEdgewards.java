...
public class ModulePlacementEdgewards extends ModulePlacement{
	...
	protected ModuleMapping moduleMapping; //instance used in vrGAME for mapping
	protected List<Sensor> sensors; //multiple sensors and actuators etc
	protected List<Actuator> actuators;
	protected Map<Integer, Double> currentCpuLoad; // Load Balancing in fog
	
	/**
	 * Stores the current mapping of application modules to fog devices 
	 */
	protected Map<Integer, List<String>> currentModuleMap; // Current map being drawn through code
	protected Map<Integer, Map<String, Double>> currentModuleLoadMap; // Mapping us always in the form of Map
	protected Map<Integer, Map<String, Integer>> currentModuleInstanceNum; //number of mapped instances
	...
	public ModulePlacementEdgewards(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators, 
		...
		//setting devices in the mapping 
		this.setFogDevices(fogDevices);
		this.setApplication(application);
		this.setModuleMapping(moduleMapping);
		this.setModuleToDeviceMap(new HashMap<String, List<Integer>>()); // Keys are set against every map for unique identification 
		this.setDeviceToModuleMap(new HashMap<Integer, List<AppModule>>());
		// Map iss a collection of key-value pairs. It maps keys to values Hashmap is java's built in class used for this purpose
		setSensors(sensors);
		
		setActuators(actuators);
		//Setting all kind of maps in hashmap in the form of key value pairs
		...
		for(FogDevice dev : getFogDevices()){ //getting devices and adding algorithm
	...
	@Override
	protected void mapModules() { //mapping modules being done
		...
		List<List<Integer>> leafToRootPaths = getLeafToRootPaths(); // Creating a path with Parent child system
	...
	private List<String> getModulesToPlace(List<String> placedModules){
		Application app = getApplication(); //getting the whole application which you create like in VRgame.
	...
	protected double getRateOfSensor(String sensorType){
		for(Sensor sensor : getSensors()){
			if(sensor.getTupleType().equals(sensorType))
				return 1/sensor.getTransmitDistribution().getMeanInterTransmitTime(); //Time like EEG TRANSMISSION TIME
		}
		return 0;
	}
	
	private void placeModulesInPath(List<Integer> path) {
		if(path.size()==0)return; //if no path is drawn then draw again
		 ...
		 */
		for(AppEdge edge : getApplication().getEdges()){ //Two types of edges period or event based see the Addedge method in VRgame 3rd line.
			if(edge.isPeriodic()){
		...
		for(Integer deviceId : path){
			 ...
			 */
			boolean changed = true; //when you add more devices through code Everything need be update correctly
			while(changed){	
				//Loop runs as long as some new information is added
				changed=false;
				...
				for(AppEdge edge : rateMap.keySet()){
					...
					for(Pair<String, String> pair : map.keySet()){
						if(pair.getFirst().equals(edge.getTupleType())){ // There are incoming and outgoing tuples and edges getting their rate from geneic methods
			...
			while(modulesToPlace.size() > 0){ // If more than 0 modules then Loop runs until all modules in modulesToPlace are deployed in the path
				...
				if(upsteamDeviceId > 0){
					if(upsteamDeviceId==deviceId){
						...
						if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()){ // DIVISION of load according to rates and demands
	 ...
	 * @param deviceId
	 */
	private List<String> shiftModuleNorth(String moduleName, double cpuLoad, Integer deviceId, List<String> operatorsToPlace) { // as Two types of regions North and South
		System.out.println(CloudSim.getEntityName(deviceId)+" is shifting "+moduleName+" north."); // mapping north wise
	 ...
	 */
	private List<String> findModulesToShift(List<String> modules, Integer deviceId) { // if new modules added then we find them first
	...
	protected List<List<Integer>> getLeafToRootPaths(){ //getting parents in each application i.e Cloud
	...
	//all these used above OOP's setter and getter 
